  ...
  const handleMembershipDecision = async (request: MembershipRequest, approved: boolean) => {
    ...
    try {
      if (approved) {
        ...
        setRoleActionNotice(`Permintaan ${request.name} ditolak.`);
      }
      setTimeout(() => setRoleActionNotice(null), 3500);
    } catch (error) {
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-7 w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
            ...
            {memberModalTab === "roster" && (
              <div className="flex-1 overflow-y-auto pt-4 pr-1 space-y-4">
                ...
                {/* Add New Member Form */}
                {canManageMembers && <form onSubmit={handleAddMemberSubmit} className="pt-4 border-t border-slate-800 space-y-3">
